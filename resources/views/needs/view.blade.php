          ...
          <div class="relative w-full h-full max-w-md md:h-auto">
              ...
              <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  ...
                  <div class="px-6 py-6 lg:px-8">
                      <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">What do you need</h3>
                      <form class="space-y-6" method="POST" action="{{ route('needs.store') }}">
                        @csrf
                            <div>
                           ...
                           <button type="submit" id="bottone1">Post</button>
                      </form>
                  </div>
              </div>
          </div>
      </div>
      <div class="flex items-end justify-end fixed bottom-2 right-2 mb-4 mr-4 z-10">
        <div>
  ...
  <div id="authentication-modal{{$need->id}}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
      <div class="relative w-full h-full max-w-md md:h-auto">
          ...
          <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
              ...
              <div class="px-6 py-6 lg:px-8">
                  <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">give him a {{ $need->label }}</h3>
                  <form class="space-y-6" method="POST" action="{{ route('donation.add') }}" enctype="multipart/form-data">
                    @csrf
                        <input type="text" class="hidden" name="id" value="{{$need->id}}">
                      <div>              
                      ...
                      <div class="my-4">
                        <x-input-label for="image" :value="__('the '.$need->label.' image')" />
                      ...
                      {{-- <button type="submit" class="w-full text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Donate</button> --}}
                  </form>
              </div>
          </div>
      </div>
